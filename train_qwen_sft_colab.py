"""
Colab-friendly SFT / QLoRA training script for the claim-judging stage.

Expected input dataset:
    factcheck_sft_1000.json
...
3. Upload this file + your JSON training data
4. Run:
   !python train_qwen_sft_colab.py \
       --train-file factcheck_sft_1000.json \
       --model-name Qwen/Qwen2.5-7B-Instruct \
       --output-dir qwen-judge-lora

Notes:
- On Colab, prefer LoRA / QLoRA, not full fine-tuning.
- Qwen 7B full fine-tuning is generally not realistic on ordinary Colab GPU RAM.
"""
...
DEFAULT_OUTPUT_DIR = "qwen-judge-lora"
...
def build_prompt(record: Dict[str, Any]) -> str:
    instruction = str(record.get("instruction", "")).strip() or (
        "Judge claim truthfulness using evidence. Output strict JSON only."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train Qwen judge model with LoRA/QLoRA on Colab.")
